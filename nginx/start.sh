#!/bin/sh

echo "setting nginx conf ..."
echo "FRONTEND_SERVER_NAME": $FRONTEND_SERVER_NAME
echo "BACKEND_GATEWAY": $BACKEND_GATEWAY
echo "ADMIN_GATEWAY": $ADMIN_GATEWAY
echo "API_PLACEHOLDER": $API_PLACEHOLDER
echo "ADMIN_PLACEHOLDER": $ADMIN_PLACEHOLDER
echo "DEVELOP_PLACEHOLDER": $DEVELOP_PLACEHOLDER
echo "DEVELOP_GATEWAY": $DEVELOP_GATEWAY
echo "UPLOADS_PLACEHOLDER": $UPLOADS_PLACEHOLDER
echo "FTP_FILES_PLACEHOLDER": $FTP_FILES_PLACEHOLDER
echo "SSL_CERTIFICATE": $SSL_CERTIFICATE
echo "SSL_CERTIFICATE_KEY": $SSL_CERTIFICATE_KEY
echo "SSL_CERTIFICATE_TRUSTED": $SSL_CERTIFICATE_TRUSTED

echo "ONEC_GATEWAY": $ONEC_GATEWAY
echo "API_ONEC_PLACEHOLDER": $API_ONEC_PLACEHOLDER

# replace env for nginx conf
envsubst '$ADMIN_PLACEHOLDER $FRONTEND_SERVER_NAME $ADMIN_GATEWAY $API_PLACEHOLDER $UPLOADS_PLACEHOLDER $BACKEND_GATEWAY $DEVELOP_GATEWAY $DEVELOP_PLACEHOLDER $FTP_FILES_PLACEHOLDER $SSL_CERTIFICATE $SSL_CERTIFICATE_KEY $SSL_CERTIFICATE_TRUSTED $ONEC_GATEWAY $API_ONEC_PLACEHOLDER' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/conf.d/default.conf

# start nginx
nginx -g 'daemon off;'
exec "$@"